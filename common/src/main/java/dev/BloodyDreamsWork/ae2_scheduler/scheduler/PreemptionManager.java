...
public final class PreemptionManager {
    ...
    public static ICraftingSubmitResult offer(IGrid grid, ICraftingPlan plan,
        ...
        var schedulers = new ArrayList<AbstractSchedulerBlockEntity>(ModRegistry.schedulers(grid));
        if (schedulers.isEmpty()) {
            return null;
        }
        schedulers.sort(Comparator.comparing(AbstractSchedulerBlockEntity::getBlockPos));
    ...
    private static boolean isViableVictim(AbstractSchedulerBlockEntity scheduler, CraftingCPUCluster cluster,
