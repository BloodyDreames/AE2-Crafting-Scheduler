...
import dev.BloodyDreamsWork.ae2_scheduler.compat.Ae2Nbt;
import dev.BloodyDreamsWork.ae2_scheduler.compat.Ae2TestSupport;
import dev.BloodyDreamsWork.ae2_scheduler.registry.ModRegistry;
import dev.BloodyDreamsWork.ae2_scheduler.scheduler.CpuKey;

public class SchedulerTestPlotsBase {
    ...
    protected SchedulerTestPlotsBase() {
    }
    ...
    private static void baseNetwork(PlotBuilder plot, boolean secondCpu, int extraPlanks,
        ...
        plot.block("1 0 0", ModRegistry.schedulerBlock());
        ...
        plot.blockEntity("1 0 -2", AEBlocks.PATTERN_PROVIDER, provider -> {
            ...
            inv.addItems(Ae2TestSupport.encodeCraftingPattern(level, new Object[] {
    ...
    private static void manageAllCpus(PlotTestHelper helper) {
        ...
        for (var scheduler : ModRegistry.schedulers(grid)) {
    ...
    public static void confirmMenuKeepsBusyCpu(PlotBuilder plot) {
        var expectedPlanks = ONE_K_PLANKS_FOR_STICKS + PLANKS_FOR_TABLE;
        baseNetwork(plot, false, 0, AEBlocks.CRAFTING_STORAGE_1K, expectedPlanks);
        plot.block(MENU_HOST_POS, Ae2TestSupport.meChest());
        ...
        plot.test(helper -> {
            ...
            helper.startSequence()
                    ...
                    .thenExecute(() -> {
                        ...
                        helper.check(Ae2TestSupport.isMeChest(host),
                                "the confirmation-menu host did not form");

                        var player = Platform.getFakePlayer(helper.getLevel(), null);
                        var menu = new CraftConfirmMenu(1, player.getInventory(), (ISubMenuHost) host);
    ...
    public static void pauseResume(PlotBuilder plot) {
        ...
        plot.test(helper -> {
            ...
            helper.startSequence()
                    ...
                    .thenWaitUntil(() -> {
                        helper.assertContains(helper.getGrid(ORIGIN), Items.CRAFTING_TABLE);
                        helper.assertContains(helper.getGrid(ORIGIN), Items.STICK);
                        var storage = helper.getGrid(ORIGIN).getStorageService().getInventory();
                        var sticks = storage.extract(AEItemKey.of(Items.STICK), Long.MAX_VALUE,
                                appeng.api.config.Actionable.SIMULATE, new BaseActionSource());
                        helper.check(sticks == STICK_TARGET,
                    ...
                    .thenExecute(() -> {
                        ...
                        var storage = helper.getGrid(ORIGIN).getStorageService().getInventory();
                        helper.assertContainsNot(storage, AEItemKey.of(Items.OAK_PLANKS));
                        var tables = storage.extract(AEItemKey.of(Items.CRAFTING_TABLE), Long.MAX_VALUE,
                                appeng.api.config.Actionable.SIMULATE, new BaseActionSource());
    ...
    private static void roundTripThroughNbt(PlotTestHelper helper) {
        ...
        var ctx = Ae2Nbt.contextFor(helper.getLevel());
        var tag = new CompoundTag();
        Ae2Nbt.writeLogic(cluster.craftingLogic, tag, ctx);
        ...
        Ae2Nbt.readLogic(cluster.craftingLogic, tag, ctx);
